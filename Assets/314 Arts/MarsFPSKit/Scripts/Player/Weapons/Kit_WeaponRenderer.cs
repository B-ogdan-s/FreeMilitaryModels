...
namespace MarsFPSKit
{
    namespace Weapons
    {
        ...
        public class Kit_WeaponRenderer : MonoBehaviour
        {
            ...
            public void WeaponUpdate(Kit_ModernWeaponScript ws, WeaponControllerRuntimeData data)
            {
                    ...
                    if (ws.attachmentSlots[i].availableAttachments[enabledAttachments[i]].GetType() == typeof(Kit_AttachmentInfo))
                    {
                        ...
                        if (prefabsToUse.fpPrefab)
                        {
                            if (attachmentAndSkinSockets[i].socket)
                            {
...
#if INTEGRATION_FPV2
                                //Set shaders
                                FirstPersonView.ShaderMaterialSolution.FPV_SM_Object obj = creation.GetComponent<FirstPersonView.ShaderMaterialSolution.FPV_SM_Object>();
                                if (obj)
                                {
                                ...
                                //Set shaders
                                FirstPersonView.FPV_Object obj = creation.GetComponent<FirstPersonView.FPV_Object>();
                                if (obj)
                                {
                                    //Set as first person object
                                    obj.SetAsFirstPersonObject();
                                ...
                                {
                                    attachmentVisualBehaviours[p].Selected(pb, AttachmentUseCase.FirstPerson);
