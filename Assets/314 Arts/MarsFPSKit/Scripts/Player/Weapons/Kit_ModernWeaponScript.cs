...
namespace MarsFPSKit
{
    namespace Weapons
    {
        ...
        public class Kit_ModernWeaponScript : Kit_WeaponBase
        {
            ...
            /// <summary>
            /// Is this weapon silenced?
            /// </summary>
            public bool fireSoundWeaponIsSilenced;
            ...
            public override object SetupFirstPerson(Kit_PlayerBehaviour pb, int[] attachments)
            {
                ...
                data.cachedFireSound = fireSound;
                data.cachedFireSoundThirdPerson = fireSoundThirdPerson;
                data.cachedFireSoundThirdPersonMaxRange = fireSoundThirdPersonMaxRange;
                data.cachedFireSoundThirdPersonRolloff = fireSoundThirdPersonRolloff;
                data.isWeaponSilenced = fireSoundWeaponIsSilenced;
            ...
            public override void AnimateWeapon(Kit_PlayerBehaviour pb, object runtimeData, int id, float speed)
            {
                if (pb.isFirstPersonActive)
                {
                    if (runtimeData != null && runtimeData.GetType() == typeof(WeaponControllerRuntimeData))
                    {
                        ...
                        if (!useGenericRunAnim && pb.isFirstPersonActive && data.weaponRenderer.anim)
                        {
                            //Set empty bool
                            data.weaponRenderer.anim.SetBool("running", id == 2);

                            for (int i = 0; i < data.weaponRenderer.animAdditionals.Length; i++)
                            {
                                data.weaponRenderer.animAdditionals[i].SetBool("running", id == 2);
                            }
                        }
            ...
            public void FireOneShot(Kit_PlayerBehaviour pb, WeaponControllerRuntimeData data)
            {
                ...
                {
                    //Play sound
                    data.soundFire.PlayOneShot(data.cachedFireSound);
                    //Play fire animation
                    if (data.bulletsLeft == 1)
                    {
                        //Last fire
                ...
                {
                    //Set clip
                    pb.thirdPersonPlayerModel.soundFire.clip = data.cachedFireSoundThirdPerson;
                    //Update range
                    pb.thirdPersonPlayerModel.soundFire.maxDistance = data.cachedFireSoundThirdPersonMaxRange;
                    //Update sound rolloff
                    pb.thirdPersonPlayerModel.soundFire.SetCustomCurve(AudioSourceCurveType.CustomRolloff, data.cachedFireSoundThirdPersonRolloff);
                    //Play
                    pb.thirdPersonPlayerModel.soundFire.PlayOneShot(data.cachedFireSoundThirdPerson);
            ...
            public void FireRaycast(Kit_PlayerBehaviour pb, WeaponControllerRuntimeData data)
            {
                if (pb.main.gameInformation.fireShotsLocally || pb.photonView.IsMine)
                {
                    ...
                    if (pb.main.minimap)
                    {
                        pb.main.minimap.PlayerShoots(pb, data.isWeaponSilenced);
                    }
                }
            }
